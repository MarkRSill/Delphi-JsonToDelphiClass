...
begin
  ...
  for i := 0 to JSONValue.Count - 1 do begin
...
begin
  ...
  if JSONValue is TJSONObject then begin
    ...
    OutputStrings.Add(StringOfChar(' ', LIdent) + JSONValue.ToJSON);
end;
  ...
  for LJsonPair in LJsonObj do begin
    ...
    case LJsonType of
      ...
      jtArray: begin
        ...
        if Assigned(LJsonVal2) then begin
          LJsonType2 := GetJsonType(LJsonVal2);
          case LJsonType2 of
            jtObject: begin // needs to have a find here instead of just Create!
              LField := AParentClass.FindField(LJsonPair.JsonString.Value);
              if not Assigned(LField) then begin
                LClass := TStubClass.Create(AParentClass, LJsonPair.JsonString.Value, Self);
                TStubObjectField.Create(AParentClass, LJsonPair.JsonString.Value, LClass);
              ...
              LJsonType2 := jtUnknown; // already handled complex jtObject type
            end;
...
begin
  ...
  try
    ...
    for k := FClasses.Count - 1 downto 0 do begin
      LClass := FClasses[k];
...
begin
  ...
  for LClass in FClasses do begin
    ALines.Add('-------');
...
begin
  ...
  LMax := 0;
  for LClass in FClasses do begin
    if LClass.Name.StartsWith(ASuggestedClassName, true) then begin
      LString := Copy(LClass.Name, length(ASuggestedClassName) + 2);
      if (LString.length = 3) then begin
        if TryStrToInt(LString, LVal) then begin
          inc(LVal);
          if LVal > LMax then
            LMax := LVal;
        end;
...
function TPkgJsonMapper.GetJsonNumericType(AJsonValue: TJSONValue; CurrentType: TJsonType = jtUnknown): TJsonType;
  ...
  begin
    ...
    if Result then begin
      Val(Source, TInt, E);
      if TInt = 1 then ; //Removes warning.
      Result := (E = 0);
    end;
  end;
...
begin
  ...
  try
    if IsInt64(fieldData) then
      Result := jtInt64
    else if IsInteger(fieldData) then
      Result := jtInteger;
  except
    Result := jtUnknown;
  end;
...
begin
  ...
  //  Find max len
  for k := 0 to AItem.Count - 1 do begin
    LItem := AItem.Items[k];
    LPos := Pos(':', LItem.Text);
...
var
  ...
  LItem: TTreeViewItem;
begin
  for LField in AClass.FItems do begin
    ...
    case LField.FieldType of
      ...
      jtArray: begin
        LItem.Text := LField.Name + ': [] ' + LField.GetTypeAsString;
        if (LField as TStubArrayField).ContainedType = jtObject then begin
          InternalVisualize(LItem, (LField as TStubArrayField).FieldClass, AItemStyleLookup);
        end;
      end;
      else LItem.Text := LField.Name + ': ' + LField.GetTypeAsString;
    end;
  ...
  try
    ...
    case GetJsonType(LJsonValue) of
      ...
      jtArray: begin
        ...
        if Assigned(LJsonValue2) then begin
          LJsonType := GetJsonType(LJsonValue2);
          LClass := TStubClass.Create(FRootClass, 'Item', Self);
          if LJsonType = jtObject then begin
            LJsonType := jtUnknown; // Flag that we've already handled array of objects since we're handling every one
            TStubArrayField.Create(FRootClass, 'Items', LJsonType, LClass);
            for LJsonItem in TJSONArray(LJsonValue) do // revised to process every element of the array
              ProcessJsonObject(LJsonItem, LClass);
          end;
        end;
        if LJsonType <> jtUnknown then begin
          TStubArrayField.Create(FRootClass, 'Items', LJsonType, LClass);
...
begin
  ...
  try
    ...
    if IncludeSerializers or (FComplexItems.Count + FArrayItems.Count > 0) then begin
      LLines.Add('');
      LLines.Add(Format('{ %s }', [LClassName]));
      LLines.Add('');
    end;
    if FComplexItems.Count > 0 then begin
      LLines.Add(Format('constructor %s.Create;', [LClassName]));
      LLines.Add('begin');
      LLines.Add('  inherited;');

      for LItem in FComplexItems do begin
    ...
    if (FComplexItems.Count > 0) or (FArrayItems.Count > 0) then begin
      LLines.Add(Format('destructor %s.Destroy;', [LClassName]));

      if FArrayItems.Count > 0 then begin
        LLines.Add('var');
        for LItem in FArrayItems do begin
    ...
    if IncludeSerializers then begin
      case AJsonLibrary of
        ...
        jlGrijjy: begin
          ...
          fromJsonString := '  Result := nil;'#13#10 + '  TgoBsonSerializer.Deserialize(AJsonString, Result);';
        end;
        jlXSuper: begin
...
begin
  ...
  try
    ...
    for LItem in FItems do begin
      if (LItem.FieldType = jtUnknown) or
        ((LItem is TStubContainerField) and ((LItem as TStubContainerField).ContainedType = jtUnknown)) then
        raise EJsonMapper.CreateFmt('The property [%s] has unknown type!', [LItem.PropertyName]);

      LString := Format('  property %s: %s read %s write %s;', [LItem.PropertyName, LItem.GetTypeAsString, LItem.FieldName,
        LItem.FieldName]);
      LLines.Add('  ' + LString);
    end;
